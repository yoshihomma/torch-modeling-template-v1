...
class CfgNode(dict):
    ...
    # copy from https://github.com/rbgirshick/yacs/issues/19
    def convert_to_dict(self, key_list=[]):
        """Convert a config node to dictionary"""
        return self._convert_to_dict(self, key_list)

    def _convert_to_dict(self, cfg_node, key_list=[]):
        if not isinstance(cfg_node, CfgNode):
            ...
            cfg_dict = dict(cfg_node)
            for k, v in cfg_dict.items():
                cfg_dict[k] = self._convert_to_dict(v, key_list + [k])
            return cfg_dict
